import * as React from "react";
import * as PopoverPrimitive from "@radix-ui/react-popover";

import { cn, applyGlassEffect, applyGradientEffect, applyNeomorphismEffect, applyAnimation } from "@/lib/utils";
import { GlassEffect, GradientType, AnimationType } from "@/types/global-style-types";

export type ModernPopoverContentProps = React.ComponentPropsWithoutRef<typeof PopoverPrimitive.Content> & {
  glassEffect?: GlassEffect;
  gradientType?: GradientType;
  neomorphism?: boolean;
  animation?: AnimationType;
};

const ModernPopover = PopoverPrimitive.Root;

const ModernPopoverTrigger = PopoverPrimitive.Trigger;

const ModernPopoverContent = React.forwardRef<
  React.ElementRef<typeof PopoverPrimitive.Content>,
  ModernPopoverContentProps
>(({ 
  className, 
  glassEffect = "advanced" as GlassEffect,
  gradientType = "none",
  neomorphism = false,
  animation = "fade-in",
  align = "center", 
  sideOffset = 4, 
  ...props 
}, ref) => {
  const shouldApplyGlass = glassEffect !== "none";
  const shouldApplyGradient = gradientType !== "none";
  const shouldApplyNeomorphism = neomorphism;
  
  return (
    <PopoverPrimitive.Portal>
      <PopoverPrimitive.Content
        ref={ref}
        align={align}
        sideOffset={sideOffset}
        className={cn(
          "z-50 w-72 rounded-md border bg-popover p-4 text-popover-foreground shadow-md outline-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2",
          shouldApplyGlass && applyGlassEffect(undefined, { variant: glassEffect }),
          shouldApplyNeomorphism && applyNeomorphismEffect(undefined),
          shouldApplyGradient && applyGradientEffect(undefined, gradientType),
          applyAnimation(undefined, animation),
          className,
        )}
        {...props}
      />
    </PopoverPrimitive.Portal>
  );
});
ModernPopoverContent.displayName = PopoverPrimitive.Content.displayName;

export { ModernPopover, ModernPopoverTrigger, ModernPopoverContent };