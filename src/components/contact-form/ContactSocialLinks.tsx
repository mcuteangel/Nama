...
const ContactSocialLinks: React.FC = () => {
  ...
  return (
    <div className="space-y-3 pt-4 border-t border-gray-200 dark:border-gray-700">
      <h3 className="text-lg font-semibold text-gray-800 dark:text-gray-100">لینک‌های شبکه‌های اجتماعی</h3>
      {fields.map((item, index) => (
        <div key={item.id} className="grid grid-cols-1 md:grid-cols-3 gap-4 items-end">
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="text-gray-700 dark:text-gray-200">نوع شبکه اجتماعی</FormLabel>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem className="flex-grow">
                  <FormLabel className="text-gray-700 dark:text-gray-200">آدرس URL</FormLabel>
                  <FormControl>
                    <Input type="url" placeholder="مثال: https://linkedin.com/in/yourprofile" className="bg-white/30 dark:bg-gray-700/30 border border-white/30 dark:border-gray-600/30 text-gray-800 dark:text-gray-100 placeholder-gray-500 dark:placeholder-gray-400" {...field} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
      ...
      >
        <Plus size={16} className="me-2" /> افزودن لینک شبکه اجتماعی
      </Button>
    </div>
  );
};
