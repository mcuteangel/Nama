...
/**
 * Ultra-Modern BaseBarChart - Next-Gen Data Visualization
 *
 * Features:
 * - Advanced 3D bar effects with depth and shadows
 * - Interactive bar animations and morphing
...
const BaseBarChart: React.FC<BaseBarChartProps> = ({
  ...
  const formattedData = useMemo(() => data.map((item: BaseBarChartDataItem, index: number) => ({
    name: translationPrefix ? t(`${translationPrefix}.${item[nameKey]}`) : (item[nameKey] as string),
    count: Number(item[valueKey] || item.value || 0),
    color: (item[colorKey] || COLORS[index % COLORS.length]) as string
  ...
  const CustomTooltip: React.FC<TooltipProps<ValueType, NameType>> = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const total = formattedData.reduce((sum, d) => sum + (d.count || 0), 0);
  ...
  const onBarClick = (_: unknown, index: number) => {
            ...
            <ResponsiveContainer width="100%" height="100%">
              ...
              >
                <defs>
                  ...
                  {/* 3D effect gradients */}
                  {formattedData.map((entry, index: number) => (
                    <filter key={`bar-shadow-${index}`} id={`bar-shadow-${index}`}>
                      <feDropShadow dx="3" dy="3" stdDeviation="3" floodColor={entry.color} floodOpacity="0.3"/>
                    </filter>
                  ))}
                </defs>
                ...
                >
                  {formattedData.map((entry, index: number) => (
                    <Cell
                      ...
                      style={{
                        filter: selectedBar === index ? `url(#bar-shadow-${index})` : 'none',
                        transform: selectedBar === index ? 'scaleY(1.05)' : hoveredBar === index ? 'scaleY(1.02)' : 'scaleY(1)',
            ...
            {selectedBar !== null && (
              <div className="absolute top-4 right-4 bg-background/95 border border-border/50 rounded-xl p-4 shadow-2xl backdrop-blur-xl max-w-xs">
                <div className="flex items-center gap-3 mb-2">
                  <div
                    className="w-4 h-4 rounded-full shadow-lg"
                    style={{ backgroundColor: formattedData[selectedBar].color }}
                  />
                  <p className="font-bold">{formattedData[selectedBar].name}</p>
                </div>
