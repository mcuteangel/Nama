...
const ContactsByPreferredMethodChart: React.FC<ContactsByPreferredMethodChartProps> = ({ data }) => {
  ...
  const getMethodConfig = (method: string) => {
    const configs: Record<string, { icon: React.ComponentType<any>, colors: { primary: string, secondary: string, accent: string }, label: string }> = {
      ...
      'address': { icon: MapPin, colors: { primary: 'from-red-500 to-pink-500', secondary: 'from-red-400 to-pink-400', accent: '#EF4444' }, label: 'آدرس' },
      'other': { icon: Star, colors: { primary: 'from-gray-500 to-slate-500', secondary: 'from-gray-400 to-slate-400', accent: '#6B7280' }, label: 'سایر' }
    };
      ...
      <div className="relative">
        ...
        <div className="absolute top-4 left-4 z-10 bg-white/10 backdrop-blur-sm rounded-xl p-3 shadow-lg">
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-white font-medium">بهینه‌سازی ارتباطات</span>
          </div>
        </div>
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <div>
                    ...
                    <p className="text-white/80 text-xs">{method.count} مخاطب</p>
                  </div>
                </div>

                <div className="space-y-2">
                  <div className="flex justify-between items-center">
                    <span className="text-white/80 text-xs">کارایی</span>
                    <span className="text-white font-semibold text-sm">{method.efficiency.toFixed(1)}%</span>
                  </div>

                  <div className="flex justify-between items-center">
                    <span className="text-white/80 text-xs">روند</span>
                    <span className={`text-xs font-medium ${
                      method.trend === 'increasing' ? 'text-green-400' : 'text-blue-400'
                    }`}>
                      {method.trend === 'increasing' ? 'افزایشی' : 'پایدار'}
                    </span>
                  </div>
                </div>
                ...
                <div className="mt-3 text-center">
                  <div className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${
                    ...
                    {method.recommendation === 'optimize' ? 'بهینه' : 'ترویج'}
                  </div>
                </div>
              </div>
            );
        ...
        <div className="mt-6 p-6 bg-gradient-to-r from-blue-500/10 to-purple-500/10 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl">
          <h3 className="text-lg font-bold text-white mb-4 flex items-center gap-2">
            💡 نکات بهینه‌سازی ارتباطات
          </h3>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="p-4 bg-white/10 rounded-xl">
              <h4 className="font-semibold text-white mb-2">پیشنهادهای دیجیتال</h4>
              <ul className="text-sm text-white/80 space-y-1">
                <li>• استفاده از ایمیل‌های خودکار</li>
                <li>• پیام‌های شخصی‌سازی شده</li>
                <li>• نوتیفیکیشن‌های هوشمند</li>
              </ul>
            </div>

            <div className="p-4 bg-white/10 rounded-xl">
              <h4 className="font-semibold text-white mb-2">بهبود تجربه</h4>
              <ul className="text-sm text-white/80 space-y-1">
                <li>• پاسخگویی سریع‌تر</li>
                <li>• پیگیری منظم</li>
                <li>• تحلیل بازخوردها</li>
              </ul>
            </div>
          </div>
        </div>
